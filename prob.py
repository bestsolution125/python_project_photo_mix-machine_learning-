...
def reduce_layer(image, kernel=generatingKernel(0.4)):
    ...
    filter_img=cv2.filter2D(image, -1, kernel, borderType=cv2.BORDER_REFLECT101)
    rows,cols=image.shape

    
    result_img=np.zeros((int(math.ceil(rows/2)), int(math.ceil(cols/2))),dtype=np.float64)

    for c in range(0, cols-1):
        for r in range(0, rows-1):
            if c%2==0 and r%2==0:
                result_img[int(r/2),int(c/2)]=filter_img[r,c]
    return result_img
...
def expand_layer(image, kernel=generatingKernel(0.4)):
    ...
    rows,cols = image.shape
    temp = np.zeros(((rows*2)+1,(cols*2)+1),dtype=np.float64)

    for c in range(0, cols):
        for r in range(0, rows):
            temp[r*2,c*2] = image[r,c]*4


    result_img = cv2.filter2D(temp, -1, kernel, borderType=cv2.BORDER_REFLECT101)
...
def gaussPyramid(image, levels):
    ...
    for x in range(0,levels+1):
        if x == 0:
            ...
            result_img.append(reduce_layer(result_img[x-1]))
    return result_img
...
def laplPyramid(gaussPyr):
    ...
    for x in range(0,levels):
        if x == levels-1:
            result_img.append(gaussPyr[x])
        else:
            cropy,cropx=gaussPyr[x].shape
            expanded=expand_layer(gaussPyr[x+1])
            result_img.append(gaussPyr[x]-(expanded[0:cropy,0:cropx]))
    return result_img
...
def blend(laplPyrWhite, laplPyrBlack, gaussPyrMask):
    ...
    result_img = []
    levels = len(laplPyrWhite)

    for x in range(0, levels):
        result_img.append(np.multiply(laplPyrWhite[x], gaussPyrMask[x]) +
                    np.multiply(laplPyrBlack[x], (1 - gaussPyrMask[x])))
...
def collapse(pyramid):
    ...
    for x in range(levels-1,0,-1):
        cropy, cropx = pyramid[x].shape
        expanded = expand_layer(pyramid[x - 1])
        pyramid[x]=pyramid[x] + (expanded[0:cropy, 0:cropx])
